...
- **claude_status.rs** - Codex status detection via event log files
...
3. **Codex Badge**: claude_status.rs watches event log files → emits `Codex-status` → frontend shows badge
...
- 常に日本語で会話する
- コミット前に必ず確認を取る
- release-please のため、コミットメッセージは Conventional Commits 形式で英語で書く
  - 例: `fix: resize tab bar after display changes`
  - 例: `feat: add editor window grouping`
