...
- [ ] AC-1: エディタの設定画面やモーダルパネルが開いても、タブバーが隠れずに最前面に表示される
- [ ] AC-2: エディタ以外のアプリに切り替えた際は、タブバーが他のアプリの前面に表示されない（従来通り画面外に退避）
- [ ] AC-3: タブマネージャの設定画面を開いても、タブバーが消えずに画面上部に表示され続ける
- [ ] AC-4: 設定画面を閉じた後、タブバーが正常に動作する
- [ ] AC-5: オンボーディング画面表示中もタブバーが維持される（設定画面と同様の分離）
